...
CONTAINS
  ...
  DO iel = 1,N2D
     ...
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ! !! HERE for reading the NBI sources at every element, with regards to the nodes.
     ! here we add the
     ! tapas_sources_el = phys%tapas_sources(Mesh%T(iel, :), :)
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!! HERE
  ...
  SUBROUTINE elemental_matrices_volume(iel,Xel,Bel,fluxel,psiel,qe,ue,u0e,Jtorel,El_n,El_nn,diff_nn_Vol_el,v_nn_Vol_el,Xg_el)
    ...
    SUBROUTINE elemental_matrices_volume(iel,Xel,Bel,fluxel,omegael,q_cylel,psiel,qe,ue,u0e,Jtorel,El_n,El_nn,diff_nn_Vol_el,v_nn_Vol_el,Xg_el)
      ...
      ! Magnetic field norm and direction at element nodes
      ! ! HERE
      Bmod_nod = SQRT(Bel(:,1)**2 + Bel(:,2)**2 + Bel(:,3)**2) ! magnitute of B at element nodes
      b_nod(:,1) = Bel(:,1)/Bmod_nod
      b_nod(:,2) = Bel(:,2)/Bmod_nod
      b_nod(:,3) = Bel(:,3)/Bmod_nod
#ifdef KEQUATION
      ...
      ! HERE after the function we made, after loading the nodes at every element of the tapas sources
      ! HERE so, here we get the NBI sources at the GAUSS points!
      ! HERE we go to case 60...
      ! source_tapas_el = phys%tapas_sources(Mesh%T, iel, :), :)  @ 2D routines
      ! source_tapas_el_dens = phys%tapas_sources(Mesh%T, iel, :), 1)  @ 2D routines
      ! source_tapas_el_momentum = phys%tapas_sources(Mesh%T, iel, :), 2)  @ 2D routines
      ! source_tapas_el_ene_i = phys%tapas_sources(Mesh%T, iel, :), 3)  @ 2D routines
      ! source_tapas_el_ene_e = phys%tapas_sources(Mesh%T, iel, :), 4)  @ 2D routines
      ! sources_tapas_gauss_point = MATMUL(refElPol%N2D, source_tapas_el)

      ! Magnetic field norm and direction at Gauss points
      Bmod = MATMUL(refElPol%N2D,Bmod_nod) ! HERE
      ! refElPol%N2D : reference element shape functions in 2d
      b = MATMUL(refElPol%N2D,b_nod)
      ...
      ! Some sources for West cases ! sources here....
      IF (switch%testcase .GE. 50 .AND. switch%testcase .LE. 59) THEN
      ...
      ! Some sources for Circular cases !!!! HERE
      IF (switch%testcase .GE. 60) THEN
         ...
         ! ENDIF
            ...
            ! we need to remake load_magnetic_field_grid()
            ! to load_tapas_sources_field_grid()
            ! call HDF5_open(fname, file_id, IERR) ! this has to be done in magnetic_field.f90
            ! ip = 128
            ! jp = 128
            ! x = xy(g, 1)  !Mesh%X(i, 1)
            ! y = xy(g, 2)  ! y = Mesh%X(i, 2)
            ...
            ! ALLOCATE (yvec(ip))
            ! xvec = r2D(1, :)
            ! yvec = z2D(:, 1) ! coordinates of grid...

            ! new with shape functions
            ! we need to put here the

            ! structure_at_gauss_points
            ! force(g, 1) = ()


            ! old below instead of shape functions
            ! dens_source_tapas_in_soledge = interpolate(ip, yvec, &
            !      jp, xvec, &
            !      dens_source_tapas, &
            !      y, x, &
            !      1e-12)
            ...
            IF (switch%testcase==61) THEN
               ...
               IF (r .LE. 0.4) THEN
                  ...
                  ! this is DENSITY (g, 2) ; momentum; (g3,4) energies and g, 5 for neutrals
#ifdef TEMPERATURE
                  force(g,3) = 8*2.838272668283863e-05 !force(g,1) ! HERE
                  force(g,4) = 8*2.838272668283863e-05
#endif

                  ! how to read it :        CALL HDF5_real_reading(file_id, dt_ME, 'dt') from Models/magnetic_field.f90
                  !   SUBROUTINE load_magnetic_field_grid()
                  ! below is in lines 340 at Models/magnetic_field.f90
                  !     DO i = 1, Mesh%Nnodes
                  ! x = Mesh%X(i, 1)
                  ! y = Mesh%X(i, 2)
                  ! !!!! HERE
                  ...
                  ! If we want to put a Gaussian distribution for the Density and Energy, copy the West case.
               END IF
